...
const STATUS_DOT = {
  active: "bg-[var(--v2-positive-text)]",
  ready: "bg-[var(--v2-accent)]",
  setup: "bg-[var(--v2-warning-text)]",
};
...
function MobileProviderSelect({
  ...
  const t = useT();
  const allGrouped = [
    ...groups.active.map((p) => ({ provider: p, status: "active" })),
    ...groups.ready.map((p) => ({ provider: p, status: "ready" })),
    ...groups.setup.map((p) => ({ provider: p, status: "setup" })),
  ];
  const selected = selectedId
    ? allGrouped.find((item) => item.provider.id === selectedId)
    : null;

  return html`
    <details className="group mb-3 w-full overflow-hidden sm:hidden">
      <summary
      ...
      >
        <span className="flex min-w-0 flex-1 items-start gap-2.5">
          ...
          ${selected && selected.status === "active" && html`<span className="mt-0.5 shrink-0 font-mono text-[10px] uppercase tracking-wider text-[var(--v2-positive-text)]">active</span>`}
        </span>
        <span
          aria-hidden="true"
...
export function ProviderManagement({ settings, gatewayStatus, searchQuery = "" }) {
  ...
  // Flatten for mobile: active first, then ready, then setup.
  const mobileList = [
    ...groups.active,
    ...groups.ready,
    ...groups.setup,
  ];
  // Only show a selected provider on mobile if the user picked one
  // or if there's an active provider. Don't auto-select the first.
  const effectiveMobileId =
    mobileSelectedId ||
    state.activeProviderId ||
    null;
  const mobileProvider = effectiveMobileId
    ? mobileList.find((p) => p.id === effectiveMobileId)
    : null;
      ...
      ${state.isLoading
        ...
        : html`
            <${MobileProviderSelect}
              providers=${mobileList}
              groups=${groups}
              selectedId=${effectiveMobileId}
